...
@router.get("/{experiment_id}/assignment/{user_id}", response_model=Dict[str, Any])
...
def experiment_results(
    ...
    group_by: str = Query(default="variant", pattern="^(variant|day|hour)$"),
    attribution: str = Query(default="unique_users", pattern="^(unique_users|event_count)$"),
    control_key: Optional[str] = Query(default=None),
    db: Session = Depends(get_db),
):
    try:
        q = ResultsQuery(
